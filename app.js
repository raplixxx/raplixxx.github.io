...
(async function bootstrap(){
  try{
    ...
    console.error("Gagal memuat tampilan shared:", err);
  }
  // onAuthStateChanged di atas akan menyembunyikan splash & menampilkan layar yang sesuai.
  // Fallback: kalau dalam 6 detik onAuthStateChanged belum juga jalan (mis. koneksi ke
  // Firebase lambat/gagal), jangan cuma sembunyikan splash — tampilkan juga layar login,
  // supaya pengguna tidak terjebak di layar kosong selamanya.
  setTimeout(() => {
    if (!splashScreen.classList.contains("hidden")){
      splashScreen.classList.add("hidden");
      if (appShell.classList.contains("hidden") && loginScreen.classList.contains("hidden")){
        loginScreen.classList.remove("hidden");
        showToast("Koneksi ke server agak lambat. Coba muat ulang halaman jika tombol login tidak muncul.", "error", { duration: 6000 });
      }
    }
  }, 6000);
})();

// Tangkap error yang tidak tertangani (mis. modul Firebase gagal dimuat) supaya
// pengguna tidak terjebak di layar splash/blank tanpa penjelasan apa pun.
